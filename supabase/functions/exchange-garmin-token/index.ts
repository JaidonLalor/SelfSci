...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!clientId || !clientSecret) {
      ...
      return new Response("Server misconfiguration", { status: 500, headers: { "Access-Control-Allow-Origin": "*" } })
    }
    ...
    if (!tokenRes.ok) {
      ...
      return new Response("Garmin token exchange failed", { status: tokenRes.status, headers: { "Access-Control-Allow-Origin": "*" } })
    }
